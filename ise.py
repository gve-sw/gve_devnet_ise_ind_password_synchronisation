...
def get_password_and_ip(name):
    url = f"https://{os.environ['ISE_HOST']}/ers/config/networkdevice/name/{name}"

    user = os.environ['ISE_USER']
    ...
    password = "No password in ISE"
    ip = "999.999.999.999"
    try:
        password = device.json()['NetworkDevice']['trustsecsettings']['deviceConfigurationDeployment']['execModePassword']
    except:
        print("no pass")
    try: 
        ip = device.json()['NetworkDevice']['NetworkDeviceIPList'][0]['ipaddress']
    except:
        print('no ip')
    return [password, ip]

def get_network_devices():
    url = f"https://{os.environ['ISE_HOST']}/ers/config/networkdevice"
    ...
    devices = resp.json()['SearchResult']['resources']

    result = []
    for d in devices:
        info = get_password_and_ip(d['name'])
        result += [{
            'name' : d['name'],
            'id' : d['name'],
